...
export class ForTimeTimer extends Timer {
    ...
    bindEvents(screen) {
        ...
        const saveSettings = () => {
            this.settings = {
                targetMinutes: parseInt(targetMinutesInput.value) || 0,
                targetSeconds: 0,
    ...
    start(screen) {
        try {
            ...
            this.settings = {
                targetMinutes: isNaN(targetMinutes) ? 10 : targetMinutes, // Используем 10 только если значение не введено
    ...
    update() {
        ...
        if (this.phase === 'countdown') {
            ...
            if (this.remainingTime === 0) {
                ...
                this.phase = 'running';
                this.updateHeader('НА ВРЕМЯ', this.formatTime(this.settings.targetMinutes, this.settings.targetSeconds));
            } else {
                this.remainingTime--;
            }
